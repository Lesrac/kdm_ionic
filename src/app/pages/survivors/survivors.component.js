var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
import { Component } from '@angular/core';
import { AlertController, NavController } from 'ionic-angular';
import { KDMDataService } from '../../service/kdm-data.service';
import { SurvivorPageComponent } from '../survivor/survivor.component';
/**
 * Created by Daniel on 24.02.2017.
 */
let SurvivorsPageComponent = class SurvivorsPageComponent {
    constructor(navCtrl, alertCtrl, kdmService) {
        this.navCtrl = navCtrl;
        this.alertCtrl = alertCtrl;
        this.kdmService = kdmService;
        this.settlementId = -1;
    }
    ngOnInit() {
        this.getSettlements();
    }
    goToDetail(survivor) {
        this.navCtrl.push(SurvivorPageComponent, {
            survivor: survivor,
            settlement: this.settlement,
        }).then();
    }
    selectedSettlement(settlement) {
        if (settlement) {
            this.kdmService.getSettlement(settlement.id).then(stlmnt => this.tempSettlement = stlmnt);
        }
    }
    confirmChange() {
        if (this.tempSettlement) {
            this.settlement = this.tempSettlement;
            this.settlementId = this.settlement.id;
        }
    }
    addSurvivor() {
        this.kdmService.createAndAddSurvivor(this.settlement);
    }
    survivorsCheck() {
        return (this.settlement &&
            this.settlement.survivors.filter(survivor => survivor.isAlive).length > this.settlement.population);
    }
    removeSurvivor(survivor) {
        const index = this.settlement.survivors.findIndex(s => survivor === s);
        if (index >= 0) {
            const alert = this.alertCtrl.create({
                title: 'Confirm deletion',
                message: 'Do you want to delete ' + survivor.name + '?',
                buttons: [
                    {
                        text: 'Cancel',
                        role: 'cancel',
                        handler: () => {
                        },
                    },
                    {
                        text: 'Yes',
                        handler: () => {
                            this.settlement.survivors.splice(index, 1);
                        },
                    },
                ],
            });
            alert.present();
        }
    }
    getSettlements() {
        this.kdmService.getSettlements().then(settlements => {
            if (settlements) {
                this.settlements = settlements;
            }
            else {
                this.settlements = [];
            }
        });
    }
};
SurvivorsPageComponent = __decorate([
    Component({
        selector: 'kdmf-page-survivors',
        templateUrl: 'survivors.component.html',
    }),
    __metadata("design:paramtypes", [NavController, AlertController, KDMDataService])
], SurvivorsPageComponent);
export { SurvivorsPageComponent };
//# sourceMappingURL=survivors.component.js.map
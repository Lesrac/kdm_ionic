...
describe('Pipe: textFormattingPipe', () => {
    it('Keep Text', inject([DomSanitizer], (domSanitizer) => {
        const pipe = new TextFormattingPipe(domSanitizer);
        const text = 'This text keeps the same. There are no changes (because) nothings there to ' +
    ...
    it('Keep Text, that does not need this pipe', inject([DomSanitizer], (domSanitizer) => {
        const pipe = new TextFormattingPipe(domSanitizer);
        const text = 'This text keeps the same. There are no changes because nothings there to ' +
            'be changed. not a single Element <because> everything is fine </because>! good.';
        const safeResourceHTML = pipe.transform(text);
    ...
    it('Replace ()', inject([DomSanitizer], (domSanitizer) => {
        const pipe = new TextFormattingPipe(domSanitizer);
        const text = 'All the ()() need to be replaced.' +
            ' (Because) () are not allowed here.';
        const replacedText = 'All the  need to be replaced.' +
            ' (Because)  are not allowed here.';
        const safeResourceHTML = pipe.transform(text);
        const sanitizedValue = domSanitizer.sanitize(SecurityContext.HTML, safeResourceHTML);
        expect(sanitizedValue).toEqual(replacedText);
    }));
    it('Replace Milestones', inject([DomSanitizer], (domSanitizer) => {
        const pipe = new TextFormattingPipe(domSanitizer);
        const text = 'All the (milestone)(milestone) need to be replaced.' +
            ' (Because) (milestone) are not allowed here.';
        const replacedText = 'All the <br/><br/><ion-icon name="book" role="img" class="icon icon-md ion-md-book"' +
    ...
    it('Replace Milestones, milestone after milestone always gets <br/>', inject([DomSanitizer], (domSanitizer) => {
        const pipe = new TextFormattingPipe(domSanitizer);
        const text = 'All the (milestone)(milestone) need to be replaced.' +
            ' Because (milestone) are not allowed here.';
        const replacedText = 'All the <br/><br/><ion-icon name="book" role="img" ' +
    ...
    it('Replace MilestoneNLs', inject([DomSanitizer], (domSanitizer) => {
        const pipe = new TextFormattingPipe(domSanitizer);
        const text = 'All the (milestoneNL)(milestoneNL) need to be replaced.' +
            ' (Because) (milestoneNL) are not allowed here.';
        const replacedText = 'All the <ion-icon name="book" role="img" class="icon icon-md ion-md-book"' +
            ' aria-label="book" ng-reflect-name="book"></ion-icon><ion-icon name="book" role="img" ' +
            'class="icon icon-md ion-md-book" aria-label="book" ng-reflect-name="book"></ion-icon>' +
    ...
    it('Replace *', inject([DomSanitizer], (domSanitizer) => {
        ...
        const text = 'All the (*)(*) need to be replaced.' +
            ' Because (*) are not allowed here.';
        const replacedText = 'All the <ion-icon name="star" role="img" ' +
    ...
    it('Replace shieldone', inject([DomSanitizer], (domSanitizer) => {
        const pipe = new TextFormattingPipe(domSanitizer);
        const text = 'All the (shieldone)(shieldone) need to be replaced.' +
            ' Because (shieldone) are not allowed here.';
        const replacedText = 'All the <ion-icon name="kdmf-shieldone" role="img" ' +
    ...
    it('Replace br', inject([DomSanitizer], (domSanitizer) => {
        const pipe = new TextFormattingPipe(domSanitizer);
        const text = 'All the (br)(br) need to be replaced.' +
            ' Because (br) are not allowed here.';
        const replacedText = 'All the <br/><br/><br/><br/>' +
            ' need to be replaced. Because <br/><br/> are not allowed here.';
        const safeResourceHTML = pipe.transform(text);
        const sanitizedValue = domSanitizer.sanitize(SecurityContext.HTML, safeResourceHTML);
        expect(sanitizedValue).toEqual(replacedText);
    }));
    it('Replace br1', inject([DomSanitizer], (domSanitizer) => {
        const pipe = new TextFormattingPipe(domSanitizer);
        const text = 'All the (br1)(br1) need to be replaced.' +
            ' Because (br1) are not allowed here.';
        const replacedText = 'All the <br/><br/>' +
            ' need to be replaced. Because <br/> are not allowed here.';
        const safeResourceHTML = pipe.transform(text);
        const sanitizedValue = domSanitizer.sanitize(SecurityContext.HTML, safeResourceHTML);
        expect(sanitizedValue).toEqual(replacedText);
    }));
    it('Replace numbers', inject([DomSanitizer], (domSanitizer) => {
        const pipe = new TextFormattingPipe(domSanitizer);
        const text = 'All the (10+)(1) need to be replaced.' +
            ' Because (1-5) are not allowed here.';
        const replacedText = 'All the <br/><br/><b>10+</b><br/><br/><b>1</b>' +
...
//# sourceMappingURL=text-formatting.pipe.spec.js.map